...
public class AsmAtomicMethods {

    private AsmAtomicMethods() {
    }
    ...
    public static int getRegisterNumber(String reg) {
        int regPos = -1;
        for (int i = 0, registersLength = AsmConstantsDefiner.getRegisters().length; i < registersLength; i++) {
            if (AsmConstantsDefiner.getRegisters()[i].equals(reg)) {
                regPos = i;
            }
        }
        while (regPos > 7) {
    ...
    public static boolean containsKeyWord(String keyWord) {
        for (String s : AsmConstantsDefiner.getKeyWords()) {
            if (s.equals(keyWord)) {
                return true;
            }
        }
    ...
    public static boolean containsCommand(String command) {
        return AsmConstantsDefiner.getCommands().containsKey(command);
    }

    public static boolean containsSegmentRegister(String segRegister) {
        return AsmConstantsDefiner.getSegmentRegisters().containsKey(segRegister);
    }

    //returns CS|SS|DS ...
    public static String getSegmentRegistersToRemove() {
        String s = "";
        for (Map.Entry<String, Integer> e : AsmConstantsDefiner.getSegmentRegisters().entrySet()) {
            s += "|" + e.getKey() + ":";
        }
    ...
    public static int getRegisterAddressMode(String reg) {
        ...
        for (int i = 0; i < AsmConstantsDefiner.getRegisters().length; i++) {
            if (AsmConstantsDefiner.getRegisters()[i].equals(reg)) {
                registerNumber = i;
                break;
            }
    ...
    public static boolean isRegister(String s) {
        return getRegisterNumber(s) != -1 || containsSegmentRegister(s)
                || AsmConstantsDefiner.USE_MATH_PROCESSORS && isMathRegister(s);
    }
    ...
    public static boolean isMathRegister(String register) {
        for (String s : AsmConstantsDefiner.getMathRegisters()) {
            ...
            if (moreZero) {
                if (reverse) {
                    if (radix == 4) {
                        ...
                        String result = String.format("%016X", (AsmConstantsDefiner.BYTES_8.subtract(number).add(AsmConstantsDefiner.ONE)));
                ...
                if (radix == 1) {
                    return AsmConstantsDefiner.BYTES_1.subtract(number).add(AsmConstantsDefiner.ONE);
                } else if (radix == 2) {
                    return AsmConstantsDefiner.BYTES_2.subtract(number).add(AsmConstantsDefiner.ONE);
                } else if (radix == 4) {
                    String result = String.format("%08X", (AsmConstantsDefiner.BYTES_4.subtract(number).add(AsmConstantsDefiner.ONE)));
                    if (reverse) {
                        return new BigInteger(result.substring(6, 8) + result.substring(4, 6) + result.substring(2, 4) + result.substring(0, 2), 16);
                    } else {
    ...
    public static List<String> readFIle(String fileName) throws IOException {
        ...
        BufferedReader reader = new BufferedReader(new FileReader(AsmConstantsDefiner.RESOURCES_PATH + fileName + ".asm"));
        String line;
    ...
    public static void writeFile(String fileName, List<AsmLineFormat> lines, List<AsmLexemTokenizer> identifiers) throws IOException {
        File file = new File(AsmConstantsDefiner.RESOURCES_PATH + fileName + ".lst");
        if (!file.exists()) {
            file.createNewFile();
        ...
        for (AsmLexemTokenizer i : identifiers) {
            if (i.getType().equals(AsmConstantsDefiner.getKeyWords()[0])) {
        ...
        for (AsmLexemTokenizer i : identifiers) {
            if (i.getType().equals(AsmConstantsDefiner.getKeyWords()[0])) {
                //nothing
            } else if (i.getType().equals(AsmConstantsDefiner.getKeyWords()[14])) {
            ...
            } else if (i.getType().equals(AsmConstantsDefiner.getKeyWords()[16])) {
