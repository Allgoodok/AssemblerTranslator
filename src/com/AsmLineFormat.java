...
public class AsmLineFormat {
    ...
    public Integer generateOffset() {
        if (line.contains(AsmConstantsDefiner.getKeyWords()[16])) {
            return 0;
        }
        return (operationCode() ? 1 : 0) + (mrm() ? 1 : 0) + (sib() ? 1 : 0) +
                (address() ? addressSize : 0) + (operand() ? operandSize : 0) +
                (changeSegmentPrefix() ? 1 : 0) + (addressSizePrefix() ? 1 : 0) + (operandSizePrefix() ? 1 : 0) + (line.contains("JMP GO1") ? 2 : 0) +
                (line.contains("POP EAX") ? -1 : 0);
    }
