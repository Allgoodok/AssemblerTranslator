...
import static com.AsmAtomicMethods.*;
import static com.AsmConstantsDefiner.*;

public class AsmFirstSecondPassage {
        ...
        private List<AsmLexemTokenizer> identifiers = new ArrayList<>();
        private boolean second = false;

        public void analyze(String fileName) throws IOException {
            List<AsmLineFormat> lines = new ArrayList<>();
        ...
        private AsmLineFormat parseStringToLine(String stringLine) {
            AsmLineFormat line = new AsmLineFormat();
        ...
        private AsmLineFormat parseIdentifier(String[] strings) {
            AsmLineFormat line = new AsmLineFormat();
            String firstWord = strings[0];
            ...
            if (firstWord.endsWith(":")) {
                ...
                if (getKeyWords()[4].equals(next)) {
                    ...
                    if (!isIdentifier(strings[2])) {
                        ...
                        line.setError(setIdentifier(firstWord, getKeyWords()[9], ZERO));
                        if (second) {
                            AsmLexemTokenizer i = getIdentifierByName(strings[2]);
                            if (i != null) {
                    ...
                    if (!isIdentifier(strings[2])) {
                        ...
                        line.setError(setIdentifier(firstWord, getKeyWords()[10], ZERO));
                        if (second) {
                            AsmLexemTokenizer i = getIdentifierByName(strings[2]);
                            if (i != null) {
                    ...
                    AsmLexemTokenizer i = getIdentifierByName(firstWord);
                    if (i != null) {
                        i.setClosed(true);
                        i.setLength(offset - i.getOffset());
                        if ("CS".equals(findCurrentSegment().getSegmentRegister())) {
                            i.setLength(offset - i.getOffset() + 2);
                        }
                    } else {
        ...
        private AsmLineFormat parseCommand(String[] strings) {
            AsmLineFormat line = new AsmLineFormat();
            //mov, add ...
            ...
            if (rule == null) {
                ...
                if (Arrays.equals(rule, REG_MEM)) {
                    ...
                    if (!isIdentifier(strings[2])) {
                        ...
                        AsmLexemTokenizer i = getIdentifierByName(identifierName);
                        if (i != null) {
                    ...
                    if (second) {
                        ...
                        if (!isIdentifier(name)) {
                            ...
                            AsmLexemTokenizer label = getIdentifierByName(name);
                            if (label == null) {
                                ...
                                if (label.getAttribute().equals(findCurrentSegment().getName())) {
                                    if (offset >= label.getOffset()) {
                                        ...
                                        line.setAddress(0xFF - offset + label.getOffset());
                                    } else {
                                        if (command.equals("JMP")) {
                                            ...
                                            line.setAddress(label.getOffset() - offset - 1);
                                            line.setAddressSize(1);
                                    ...
                                    if (command.equals("JMP")) {
                                        label = getIdentifierByName(name);
                                        if (name.equals("GO1")) {
                                            line.setOperationCode(0xEA);
                                            line.setAddress(label.getOffset());
                                            line.setAddressSize(4);
                                        } else if (name.equals("GOADDR")) {
                                            line.setOperationCode(0xFF);
                                            line.setMrm(0x25);
                                            line.setAddress(label.getOffset());
                                            line.setAddressSize(4);
                                        }
        ...
        private AsmLineFormat parseKeyWord(String[] strings) {
            AsmLineFormat line = new AsmLineFormat();
            ...
            if (getKeyWords()[13].equals(firstWord)) {
                ...
                for (int index = 1; index < strings.length; index++) {
                    ...
                    AsmLexemTokenizer i = getIdentifierByName(s[1]);
                ...
                //if
                openedIf = true;
                AsmLexemTokenizer i = getIdentifierByName(strings[1]);
                if (i == null) {
                    line.setError("Unknown identifier");
                } else {
        ...
        public AsmLineFormat generateMrmSib(String[] data, String[] rule) {
            AsmLineFormat line = new AsmLineFormat();
            int mod = 0;
            int reg;
            int rm = 0;
        ...
        private String setIdentifier(String name, String type, BigInteger value) {
            ...
            AsmLexemTokenizer identifier = findCurrentSegment();
            if (identifier == null) {
                return "Defined variable before segment opening";
            }

            if (second) {
                for (AsmLexemTokenizer i : identifiers) {
                    if (i.getName().equals(name) && i.getType().equals("ERROR")) {
                        return "Redefined identifier";
                    }
                }
            } else {
                AsmLexemTokenizer i = getIdentifierByName(name);
                if (i != null) {
                    identifiers.add(new AsmLexemTokenizer(name, "ERROR", offset, identifier.getName()));
                    return "Redefined identifier";
                }
                if (getKeyWords()[16].equals(type)) {
                    identifiers.add(new AsmLexemTokenizer(name, type, offset, value, identifier.getName()));
                } else {
                    identifiers.add(new AsmLexemTokenizer(name, type, offset, identifier.getName()));
                }
            }
        ...
        private String setSegment(String name, String type) {
            AsmLexemTokenizer identifier = getIdentifierByName(name);
            if (!second && identifier != null) {
                identifiers.add(new AsmLexemTokenizer(name, "ERROR", offset, 0, false));
                return "Redefined identifier";
            }
            if (!second) {
                identifiers.add(new AsmLexemTokenizer(name, type, offset, 0, false));
            } else {
        ...
        private String closeSegment() {
            AsmLexemTokenizer i = findCurrentSegment();
            if (i != null) {
                i.setLength(offset - i.getOffset());
                i.setClosed(true);
            } else {
        ...
        private Integer checkIdentifierForPrefixChangeSegment(String identifierName) {
            AsmLexemTokenizer i = getIdentifierByName(identifierName);
            AsmLexemTokenizer segment = findCurrentSegment();
            if (i == null || segment == null || !PREFIX_SEGMENT_OBVIOUSLY) {
                return null;
            }
            AsmLexemTokenizer identifiersSegment = getIdentifierByName(i.getAttribute());
        ...
        private AsmLexemTokenizer findCurrentSegment() {
            for (AsmLexemTokenizer i : identifiers) {
        ...
        private AsmLexemTokenizer getIdentifierByName(String name) {
            for (AsmLexemTokenizer i : identifiers) {
